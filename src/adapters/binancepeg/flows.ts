...
const intervalSize = 14000; // size of interval in blocks to calculate inflows/outflows on
...
const bscPegTokenFlows = () => {
  return async (fromBlock: number, toBlock: number) => {
    ...
    /*
      Attempt to track burn events using the burn wallet I found.
      Burned assets are recorded as a negative withdrawal value in an attempt to cancel 
      the volume of the assets being moved to the burn wallet.
      The reason the burn wallet is not simply included in the list of binance wallets
      is because then, when the asset is burned, it would count as a withdrawal to BSC when
      it is actually a deposit from BSC.
    */
    ...
    const filteredBurnTxs = burnTxs
      .map((tx) => {
        if (binancePegTokens.includes(tx.token)) {
          return { ...tx, 
            txHash: `netFlowTx-${tx.txHash}`,
            amount: tx.amount.mul(-1) };
        } else return null;
      })
      .filter((tx) => tx) as EventData[];
    
    return [...transactions, ...filteredBurnTxs];
  };
};
