...
import { queryAggregatedDailyTimestampRange, queryAggregatedHourlyTimestampRange } from "./wrappa/postgres/query";
...
export const getDailyBridgeVolume = async (
  ...
  if (currentDayHourlyData?.length) {
    const lastDailyTimestamp = timestampAtStartOfDay;
    historicalDailySums[lastDailyTimestamp] = historicalDailySums[lastDailyTimestamp] || {};
    currentDayHourlyData.map((hourlyData) => {
      ...
      historicalDailySums[lastDailyTimestamp].depositUSD =
        (historicalDailySums[lastDailyTimestamp].depositUSD ?? 0) + parseFloat(total_deposited_usd);
      historicalDailySums[lastDailyTimestamp].withdrawUSD =
        (historicalDailySums[lastDailyTimestamp].withdrawUSD ?? 0) + parseFloat(total_withdrawn_usd);
      historicalDailySums[lastDailyTimestamp].depositTxs =
        (historicalDailySums[lastDailyTimestamp].depositTxs ?? 0) + total_deposit_txs;
      historicalDailySums[lastDailyTimestamp].withdrawTxs =
        (historicalDailySums[lastDailyTimestamp].withdrawTxs ?? 0) + total_withdrawal_txs;
    });
  }

  const dailyBridgeVolume = Object.entries(historicalDailySums).map(([timestamp, data]) => {
    return {
      date: timestamp,
      ...data,
    };
...
export const getHourlyBridgeVolume = async (
  ...
  const hourlyBridgeVolume = Object.entries(historicalHourlySums).map(([timestamp, data]) => {
    return {
      date: timestamp,
      ...data,
    };
